...
- Ready to rerun the explicit `--normalized-drift-admission p2_one_per_family` runtime smoke now that `/private/tmp/gensr_sde_32_rolewise_no_multi_u0.log` has been regenerated and backed up.
...
9. Run the exact explicit `--normalized-drift-admission p2_one_per_family` coverage-only command before widening drift beam/top-k.
