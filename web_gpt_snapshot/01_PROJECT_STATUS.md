...
- Deciding the next scorer-side residual/calibration diagnostic after P2 score-miss residuals showed mostly active/weak traps. Do not treat the P2 coverage result as selected-rerank evidence or a production default change.
...
10. Use `p2_oracle_score_miss_residual_diagnostics.json` only for targeted residual/calibration diagnostics. Do not run formal eval or integrate P2 admission from current residual evidence.
