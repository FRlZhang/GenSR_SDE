...
- Added optional `--rerank-residual-debug-json` to `sde_validation_probe.py` and `scripts/analyze_residual_debug_safety.py`. The 8-sample smoke logged 101 candidates but had 0 oracle candidates and 0 selected hits. A follow-up 16-sample smoke logged 201 candidates, 2 oracle samples, 1 selected hit, and 1 pair-oracle sample. The offline `per_active_dim_norm_plus_weak` safety parse preserved the hit and harmed 0, but rescued 0/1 oracle-present selected misses. A standalone expanded-pairing 16-sample smoke script now exists at `scripts/run_expanded_pairing_residual_debug_smoke16.sh`.

## In Progress

- Transitioning from token recognition to full symbolic sequence recovery.
- Using residual-debug JSON smoke output to check scorer safety offline before any scorer change.
...
6. Do not add robust/clipped/per-dimension normalized active scoring as a rerank mode yet. The 16-sample residual-debug safety smoke preserved 1/1 selected hits but rescued 0/1 oracle-present misses, so no formal eval is justified. If scorer safety remains the priority, run the prepared local expanded-pairing residual-debug smoke script.
