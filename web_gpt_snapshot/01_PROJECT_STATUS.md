...
- Planning a future small local coverage-only run of the explicit `--normalized-drift-admission p2_one_per_family` flag. Do not treat this as selected-rerank evidence or a production default change.
...
9. Prefer a future small local coverage-only run of the explicit `--normalized-drift-admission p2_one_per_family` flag before widening drift beam/top-k.
