...
- Blocked on missing `/private/tmp/gensr_sde_32_rolewise_no_multi_u0.log` for the explicit `--normalized-drift-admission p2_one_per_family` runtime smoke. Restore/recreate that log locally before rerunning the exact coverage-only command.
...
9. Restore/recreate `/private/tmp/gensr_sde_32_rolewise_no_multi_u0.log`, then run the exact explicit `--normalized-drift-admission p2_one_per_family` coverage-only command before widening drift beam/top-k.
