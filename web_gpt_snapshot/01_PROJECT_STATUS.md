...
- Planning a small coverage-only implementation hook for `P2_one_per_family` capped normalized drift-only admission before widening drift beam/top-k or changing production defaults.
...
9. Prefer a small coverage-only implementation hook for `P2_one_per_family` capped normalized drift-only admission before widening drift beam/top-k.
